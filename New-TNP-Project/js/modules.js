...
function renderJobsGrid(jobs) {
    ...
    grid.innerHTML = jobs.map(job => {
        ...
        const isEligible= student.cgpa >= (parseFloat(job.eligibility) || 0);
...
function handleTrainingRegistration(trnId, btn) {
    const student = mockData.currentStudent;
...
function renderUniversitiesGrid(unis) {
    ...
    grid.innerHTML = unis.map(uni => {
        ...
        const isEligible     = student.cgpa >= (parseFloat(uni.minCGPA) || 0);
            ...
            <div class="job-card">
                ...
                <div class="space-y-1.5 text-[11px]">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-semibold text-[var(--text-primary)] text-right max-w-[60%] truncate">${uni.program}</span>
                    </div>
                    <div class="flex items-center justify-between">
...
function handleUniversityApply(uniId, btn) {
    const student = mockData.currentStudent;
    const uni     = mockData.universities.find(u => u.id === uniId);
    if (!uni || student.universityApplications.includes(uniId)) return;

    if (student.cgpa < uni.minCGPA) {
        app.showToast(`You don't meet the CGPA requirement for ${uni.name}.`, "danger");
        return;
    }
