...
function renderReportsDashboard(container) {
    ...
    // Render all charts — wrapped in try/catch to guard against CDN failures
    setTimeout(() => {
        const deptData = mockData.analytics.departmentPlacements;
        // Compute percentage array if not present (fallback safety)
        const pctData = deptData.placementPct ||
            deptData.placedCount.map((p, i) => Math.round((p / deptData.totalCount[i]) * 100));
        ...
        try {
            new ApexCharts(document.querySelector("#chart-rpt-dept"), {
                ...
                labels: deptData.branches.slice(0, 4), // radialBar works best with ≤4 series
                colors: ["#4F46E5","#7C3AED","#06B6D4","#10B981"],
                legend: { show: true, position: "bottom", fontSize: "11px" },
                stroke: { lineCap: "round" }
            }).render();
        } catch(e) { console.warn("Reports dept chart error:", e); }
        ...
        } catch(e) { console.warn("Reports package chart error:", e); }
        ...
        } catch(e) { console.warn("Reports industry chart error:", e); }
        ...
        } catch(e) { console.warn("Reports outcome chart error:", e); }
    }, 200);
}
