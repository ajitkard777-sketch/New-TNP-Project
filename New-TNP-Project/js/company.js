...
function filterCandidates() {
    ...
    const filtered = mockData.applications.filter(app => {
        if (app.companyName.toLowerCase() !== cName.toLowerCase()) return false;
        const matchQ = app.studentName.toLowerCase().includes(query) ||
                       app.studentBranch.toLowerCase().includes(query) ||
                       app.jobTitle.toLowerCase().includes(query);
        return matchQ && (!status || app.status === status);
    });
