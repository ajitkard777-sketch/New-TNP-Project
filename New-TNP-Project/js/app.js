...
const app = {
    ...
    // Track the active view so theme-toggle can re-render it correctly
    currentView: "landing",
    ...
    init() {
        ...
        // ── PHP Session Bridge ────────────────────────────────
        // When served via index.php, window.__tpmsAuth is injected
        // with the logged-in role. Auto-navigate to the dashboard.
        if (window.__tpmsAuth && window.__tpmsAuth.role) {
            const auth = window.__tpmsAuth;
            mockData.session.role   = auth.role;
            mockData.session.userId = String(auth.id || '');
            if (auth.role === 'student') {
                mockData.currentStudent.name  = auth.name  || mockData.currentStudent.name;
                mockData.currentStudent.email = auth.email || mockData.currentStudent.email;
            }
            this.navigate('dashboard');
            return;
        }
        // ── Fallback: SPA-only mode (index.html direct) ───────
    ...
    navigate(view) {
        ...
        // Track current view for theme-toggle re-render
    ...
    logout(e) {
        ...
        // If PHP session is active, do a server-side logout
        if (window.__tpmsAuth) {
            window.location.href = 'logout.php';
            return;
        }
        // SPA-only mode fallback
    ...
    toggleTheme() {
        ...
        // Re-render current view to update chart colours — use tracked view, not hardcoded
        const state = mockData.session;
        if (state.role !== "guest") {
